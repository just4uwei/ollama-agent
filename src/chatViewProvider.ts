...
export class ChatViewProvider implements vscode.WebviewViewProvider {
    ...
    private _lastUserFiles: string[] = [];
    private _lastUserImages: string[] = [];
    ...
    private _clearCurrentSession() {
        const session = this._getActiveSession();
        if (session) { session.messages = []; session.title = '新对话'; session.updatedAt = Date.now(); }
    }
    ...
    private _selectModel(images?: string[]): string {
        if (images && images.length > 0 && this._visionModel) { return this._visionModel; }
        return this._currentModel;
    }

    private _parseHandoff(response: string): { target: 'vision' | 'text'; prompt: string } | null {
        const match = /```handoff\s*\n([\s\S]*?)```/g.exec(response);
        if (!match) { return null; }
        try {
            const parsed = JSON.parse(match[1].trim());
            if (parsed.target && parsed.prompt) { return { target: parsed.target, prompt: parsed.prompt }; }
        } catch {}
        return null;
    }

    private _isAborted(): boolean { return !!this._abortController?.signal.aborted; }

    private async handleUserMessage(message: string, filePaths?: string[], images?: string[]) {
        if (!this._view) { return; }
        const session = this._getActiveSession();
        if (!session) { return; }
        ...
        const userMsg: ChatMessage = { role: 'user', content: message, model: useModel };
        if (filePaths && filePaths.length > 0) { userMsg.files = filePaths; }
        ...
        try {
            ...
            while (handoff && handoffCount < MAX_HANDOFFS && !this._isAborted()) {
                ...
                if (!targetModel || targetModel === previousModel) { break; }
                ...
                const handoffResponse = await this.processWithAgent(
                    handoff.prompt, undefined,
                    isVisionTarget ? images : undefined, targetModel
                );
    ...
    private _stripToolBlocks(response: string): string {
     ...
     */
    private async _executeAndDisplayTools(response: string, model: string): Promise<void> {
    ...
    private _parseToolCalls(response: string): ToolCall[] {
    ...
    private async processWithAgent(message: string, filePaths?: string[], images?: string[], useModel?: string): Promise<string> {
...
不需要 handoff 时正常回答即可。`;
        ...
        if (hasVision && images) {
            ...
            if (session) {
                for (const msg of session.messages.slice(-10)) {
                    ...
                    if (msg.images && msg.images.length > 0 && this._isVisionModel(model)) { m.images = msg.images; }
                    messages.push(m);
                }
            }
        }
        ...
        if (wantsSearch) {
            try {
                ...
                if (relevantCode) {
                    ...
                    if (lastUserMsg) { lastUserMsg.content = `相关代码片段:\n${relevantCode}\n\n用户问题: ${message}`; }
                }
            } catch (e) {}
        }
    ...
    private async indexProject() {
        if (!vscode.workspace.workspaceFolders || vscode.workspace.workspaceFolders.length === 0) {
            vscode.window.showWarningMessage('请先打开一个文件夹'); return;
        }
